<template>
  ...
  >
    ...
    <div class="fixed top-0 z-40 w-full">
      <div
        class="w-full translate-y-0 transform transition-all duration-200 ease-in-out"
        :class="[
          'bg-primary-lightest',
          {
            'fixedHeader fixed top-0 z-40 transform': fixed,
          },
        ]"
      >
        <header
        ...
        >
          ...
          <div class="container relative z-20">
            <div class="nav-menu flex items-stretch justify-between">
              <!-- Logo -->
              <div class="py-3 lg:w-1/4">
                <NuxtLink :to="localePath(resolveUrl({ type: 'home' }))">
                  <img
                    ...
                    :style="{ height: header.logoHeight + 'px' }"
                    class="logo inline-block w-auto"
                    :alt="storeName"
                  />
                  <span
                    v-else
                    class="whitespace-no-wrap text-3xl sm:text-4xl"
                    >{{ storeName }}</span
                  >
                </NuxtLink>
              </div>
              ...
              <nav v-if="menu" class="hidden w-full lg:flex lg:w-auto">
                <ul class="flex justify-center">
                  ...
                  >
                    <v-btn
                      v-if="item.type === 'url'"
                      class="sw-nav-link relative mt-5 flex items-center px-5 py-1"
                    ...
                    >
                      ...
                      <span class="v-ripple__container"></span>
                    </NuxtLink>
                    <!-- Show mega nav if item has child items -->
                    <transition name="fade">
                      <div
                        v-if="megaNavIsActive(item, index)"
                        class="dropdownNav absolute left-0 right-0 min-h-full"
                  ...
                  <li>
                    <NuxtLink
                      ...
                      class="sw-nav-link relative mt-5 flex items-center px-5 py-1"
                    >
                      <span class="relative"> Quotation </span>
                    </NuxtLink>
                  </li>
                  <!-- <li>
                     <NuxtLink
              ...
              <!-- Action menu -->
              <div
                class="-mr-2 flex flex-row items-center justify-end lg:w-1/4"
              >
                <!-- Locale select -->
                <LocaleSelect
                ...
                <!-- Search icon -->
                <nav class="user-menu">
                  <v-btn v-if="isDesktop" href="/account/login/" text>
                    login
                  </v-btn>
                  <v-btn
                    v-if="isDesktop"
